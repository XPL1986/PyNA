...
def dataframe_to_excel(filename, df):
    output = "/Users/lcwong/Desktop/PyNA/python/output/pipeline_3/" + filename + ".xlsx"
...
def tumor_freq(df, save=False):
    df = df[((df['VAF_t'] <= .6) & (df['DP_t'] >= 10)) & ((df['VAF_t'] >= .4) & (df['DP_t'] >= 10))]
...
def run():
    ...
    biotype = pd.read_excel(open('/Users/lcwong/Desktop/PyNA/python/output/pipeline_3/protein_coding.xlsx', 'rb'),
                               sheet_name='Sheet1')
