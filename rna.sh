...
#cd ${HOME}/data/${pid}
#input=/Volumes/qmu/NG2016/rGBM_RNA/${pid}.bam
#output=${HOME}/data/${pid}/${pid}.sort.bam
#samtools sort $input -@ 8 -o $output
#samtools index ${pid}.sorted.bam
...
java -jar $picardDir AddOrReplaceReadGroups I=$input O=$output SO=coordinate RGID=id RGLB=library RGPL=platform RGPU=machine RGSM=sample
...
# 3. Split'N'Trim and reassign mapping qualities ||GATK requires Java 1.8 and reference files||
export JAVA_HOME=`/usr/libexec/java_home -v 1.8`
#ref=${HOME}/bin/hg38/hg38.fa
ref=${HOME}/bin/GRCh37/reference.fa

gatkDir=${HOME}/bin/gatk/GenomeAnalysisTK.jar
outputDir=${HOME}/data/${pid}/step3 && mkdir $outputDir

input=$output
output=${outputDir}/split.bam
java -jar $gatkDir -T SplitNCigarReads -R $ref -I $input -o $output -rf ReassignOneMappingQuality -RMQF 255 -RMQT 60 -U ALLOW_N_CIGAR_READS
...
java -Xmx8g -jar $gatkDir -T HaplotypeCaller -R $ref -I $input -o $output -dontUseSoftClippedBases -stand_call_conf 20.0
...
input=$output
output=${outputDir}/output.vcf
java -jar $gatkDir -T VariantFiltration -R $ref -V $input -window 35 -cluster 3 -filterName FS -filter "FS > 30.0" -filterName QD -filter "QD < 2.0" -o $output
